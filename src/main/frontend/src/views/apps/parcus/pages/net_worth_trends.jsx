...
function NetWorthTrends() {
  ...
  const fetchData = async (startDate, endDate) => {
    ...
    const promise = axios({
      url: `${import.meta.env.VITE_BASE_API_URL}/api/insights/net-worth-trends?${params}`,
      method: "GET",
  ...
  return (
    <>
      <div className="#bg-gray-50 flex size-full flex-col gap-4 overflow-y-scroll p-4">
        <div className="rounded-xl border-2 border-gray-200 py-2 pl-0 pr-4">
          <ResponsiveContainer height={700} width="100%">
            <AreaChart className="pl-4" data={chartData}>
              ...
              <Area dataKey="amount" fill="url(#colorNet)" stroke="#0073BF" strokeWidth={2} type="monotone" />
...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="#bg-gray-50 rounded-xl border-1 border-gray-300 p-2">
        <div>
          <div style={{ display: "inline-block", padding: 10 }}>
            <div>Bersih</div>
            <div>{Utils.getIndonesianCurrency(Math.round(payload[0].payload.amount))}</div>
          </div>
        </div>
      </div>
    )
