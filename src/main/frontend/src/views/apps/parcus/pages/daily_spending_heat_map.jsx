...
function DailySpendingHeatMap() {
  ...
  const { data } = useSWR([`${import.meta.env.VITE_BASE_API_URL}/api/insights/daily-spending-heat-map`, { ...filterData }], Utils.fetcher)
  ...
  return (
    <>
      <div className="flex size-full flex-col gap-4 overflow-y-scroll #bg-gray-50 p-4">
        ...
        <BaseCard className="flex h-[40rem] flex-none flex-row gap-8 p-4">
          {[1, 2, 3, 4, 5, 6, 7].map((e) => {
            return (
              <div className="flex size-full flex-col items-center justify-end gap-2" key={e}>
                <p className="text-lg text-gray-600">{Utils.formatToCompact(data?.data?.[e])}</p>
                <div className="w-full rounded-lg bg-blue-400" style={{ height: `${data?.data?.[e] / highestAmountOnDay * 100}%` }}></div>
                <p className="text-lg text-gray-900">{Utils.numberToIndonesianDate(e)}</p>
              </div>
            )
          })}
        </BaseCard>
