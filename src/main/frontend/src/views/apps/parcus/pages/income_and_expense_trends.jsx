...
function IncomeAndExpenseTrends() {
  ...
  const fetchData = async (startDate, endDate) => {
    ...
    const promise = axios({
      url: `${import.meta.env.VITE_BASE_API_URL}/api/insights/income-and-expense-trends?${params}`,
      method: "GET",
    ...
    const temp = []
    Object.keys(raw.netCashFlowMap).forEach((key) => {
      temp.push({
        name: key,
        // TODO: FIX NET AMOUNT IN BACKEND
        netAmount: raw.expenseCashFlowMap[key] + raw.incomeCashFlowMap[key],
        expenseAmount: raw.expenseCashFlowMap[key],
        incomeAmount: raw.incomeCashFlowMap[key],
            ...
            <AreaChart className="pl-4" data={chartData}>
              <defs>
                ...
                <linearGradient id="colorIncome" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="5%" stopColor="#66C2A4" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#36A261" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="colorExpense" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="5%" stopColor="#F27863" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#E55934" stopOpacity={0} />
                </linearGradient>
              </defs>

              <RechartsTooltip content={<CustomTooltip />} />
              <Area dataKey="netAmount" fill="url(#colorNet)" stroke="#0073BF" strokeWidth={2} type="monotone" />
              <Area dataKey="expenseAmount" fill="url(#colorExpense)" stroke="#E55934" strokeWidth={2} type="monotone" />
              <Area dataKey="incomeAmount" fill="url(#colorIncome)" stroke="#36A261" strokeWidth={2} type="monotone" />
            ...
            <div className="flex h-full flex-row justify-start gap-4">
              <div className="flex w-full flex-col items-center justify-end">
                <Tooltip content={Utils.getIndonesianCurrency(data?.keyData?.totalIncome ?? 0)}>
                  <div className="w-full rounded-lg bg-blue-400" style={{ height: `${(data?.keyData?.totalIncome ?? 0) / Math.max((data?.keyData?.totalIncome ?? 0), (data?.keyData?.totalExpense ?? 0)) * 100}%` }}></div>
                </Tooltip>
                <IconArrowDown className="mt-4 stroke-gray-600" />
              </div>
              <div className="flex w-full flex-col items-center justify-end">
                <Tooltip content={Utils.getIndonesianCurrency(data?.keyData?.totalExpense ?? 0)}>
                  <div className="w-full rounded-lg bg-orange-400" style={{ height: `${(data?.keyData?.totalExpense ?? 0) / Math.max((data?.keyData?.totalIncome ?? 0), (data?.keyData?.totalExpense ?? 0)) * 100}%` }}></div>
                </Tooltip>
      ...
      <div className="rounded-xl border-1 border-gray-300 #bg-gray-50 p-2">
        <div>
          <div style={{ display: "inline-block", padding: 10 }}>
            <div>Pengeluaran</div>
            <div>{Utils.getIndonesianCurrency(Math.round(payload[0].payload.expenseAmount))}</div>
          </div>
          <div style={{ display: "inline-block", padding: 10 }}>
            <div>Pemasukan</div>
            <div>{Utils.getIndonesianCurrency(Math.round(payload[0].payload.incomeAmount))}</div>
          </div>
          <div style={{ display: "inline-block", padding: 10 }}>
            <div>Bersih</div>
            <div>{Utils.getIndonesianCurrency(Math.round(payload[0].payload.netAmount))}</div>
          </div>
        </div>
      </div>
    )
...
export default IncomeAndExpenseTrends
