...
function ExpenseRatioTrend() {
  const [dateFilter, setDateFilter] = useState("YTD")
  const [chartData, setChartData] = useState([])
  const [yAxisWidth, setYAxisWidth] = useState(70)

  const { data } = useSWR([`${import.meta.env.VITE_BASE_API_URL}/api/insights/expense-ratio`, { dateFilter }], Utils.fetcher)
  async function processRawData(rawData) {
    const temp = []
    Object.keys(rawData.expenseRatios).forEach((key) => {
      temp.push({
        name: key,
        ratio: rawData.expenseRatios[key],
            ...
            <AreaChart className="pl-4" data={chartData}>
              <defs>
                <linearGradient id="colorNet" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="5%" stopColor="#fab169" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#bf7300" stopOpacity={0} />
                </linearGradient>
              </defs>

              <RechartsTooltip />
              <Area dataKey="ratio" fill="url(#colorNet)" stroke="#bf7300" strokeWidth={2} type="monotone" />
...
export default ExpenseRatioTrend